...
if (args.includes("--help") || args.includes("-h")) {
  console.log(
    [
      ...
      "Final external evidence artifacts must share one QA run date across enabled endpoints, rollback endpoints, analytics, entry surfaces, and rollback owner handoff.",
      "Use --final after real-device evidence is filled; it exits non-zero unless the run sheet, matrix, packet, launch run plan, enabled endpoint artifact, rollback endpoint artifact, analytics evidence, entry surface evidence, and rollback owner handoff are ready.",
...
function messagesForNextAction(
  ...
  if (externalEvidenceDateConsistency.problemCount > 0) {
    messages.push("Fix external launch evidence dates so endpoint, analytics, entry surface, and rollback owner artifacts share one QA run date.");
  }
