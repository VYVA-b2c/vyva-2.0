...
if (args.includes("--help") || args.includes("-h")) {
  console.log(
    [
      ...
      "Final external evidence artifacts must share one QA run date across enabled endpoints, rollback endpoints, analytics, real-use evidence, entry surfaces, and rollback owner handoff.",
      "Use --final after real-device evidence is filled; it exits non-zero unless the run sheet, matrix, packet, launch run plan, enabled endpoint artifact, rollback endpoint artifact, analytics evidence, real-use evidence, entry surface evidence, and rollback owner handoff are ready.",
...
function messagesForNextAction(
  ...
  if (externalEvidenceDateConsistency.problemCount > 0) {
    messages.push("Fix external launch evidence dates so endpoint, analytics, real-use, entry surface, and rollback owner artifacts share one QA run date.");
  }
...
const realUseRun = realUsePathArg
  ...
  : null;
const entrySurfacesRun = entrySurfacesPathArg
  ? runValidator(entrySurfacesValidatorPath, `--input=${entrySurfacesPathArg}`, {
      allowPending: false,
    })
  : null;
...
const summary = {
  ...
  entrySurfaceEvidence: {
    provided: Boolean(entrySurfacesPathArg),
