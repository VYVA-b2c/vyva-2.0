...
describe("ParticipateScreen", () => {
  ...
  it("renders a calm curated events screen with large low-pressure actions", async () => {
    renderParticipate();

    expect(await screen.findByRole("heading", { name: "Events chosen for you" })).toBeInTheDocument();
    ...
    expect(screen.getByTestId("participate-profile-signals")).toHaveTextContent("music");
    expect(screen.getByTestId("participate-featured-event")).toHaveTextContent("Familiar songs table");
    expect(screen.getByTestId("participate-featured-event")).toHaveTextContent("Matches music");
  ...
  it("saves interested and maybe choices without making a commitment", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("participate-featured-event")).toHaveTextContent("Interest saved");
    });
    expect(screen.getByTestId("participate-saved-events")).toHaveTextContent("Familiar songs table");
    ...
    await waitFor(() => {
      expect(screen.getByTestId("participate-featured-event")).toHaveTextContent("Saved for later");
    });
  });
  ...
  it("invites seniors with missing profile signals to add hobbies", async () => {
    ...
    expect(await screen.findByTestId("participate-profile-nudge")).toHaveTextContent("Tell us your interests");
