...
import { AlertTriangle, CheckCircle2, ClipboardCheck, Route, ShieldCheck, Wrench } from "lucide-react";
  ...
  return (
    <tr className="align-top" data-testid={flowTestId(row.reference)}>
      ...
      <td className="min-w-[180px] border-t border-[#f0e7df] px-4 py-4">
        <div className="flex flex-col gap-2">
          <Pill tone={row.readyForUsers ? "good" : "warn"}>
            {row.readyForUsers ? "Ready" : "Needs attention"}
          </Pill>
...
export default function ConciergeReadinessAdminPage() {
  const rows = useMemo(() => buildConciergeReadinessRows(), []);
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <MetricTile
            ...
            sub="Ready status with all launch gates covered."
          ...
          <MetricTile
            ...
            sub="Any missing coverage, entry point, or incomplete status."
            testId="metric-concierge-readiness-needs-attention"
          />
          <MetricTile
            icon={ShieldCheck}
            label="Provider gated"
            value={String(summary.providerGated)}
            sub="Flows that depend on trusted provider setup."
            testId="metric-concierge-readiness-provider-gated"
          />
        </section>
          ...
          <div className="flex flex-col gap-3 border-b border-[#f0e7df] px-4 py-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm font-semibold text-[#7d6b65]">
                One row per Concierge flow, including routes, saved setup, provider gates, and tools.
              </p>
            </div>
            ...
            <table className="min-w-full border-collapse text-left">
              <thead className="bg-[#fbf8f5] text-xs font-black uppercase tracking-[0.12em] text-[#8b7a73]">
                <tr>
                  ...
                  <th className="px-4 py-3">Required setup</th>
