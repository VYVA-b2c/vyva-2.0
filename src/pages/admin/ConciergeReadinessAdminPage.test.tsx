...
describe("ConciergeReadinessAdminPage", () => {
  ...
  it("renders the internal readiness table and summary metrics", () => {
    ...
    expect(channelSection).toHaveTextContent("Live use");
    expect(channelSection).toHaveTextContent("Status");
    expect(channelSection).toHaveTextContent("Manage");
    expect(screen.getByTestId("row-concierge-channel-phone-call")).toHaveTextContent("Needs setup");
    expect(screen.getByTestId("row-concierge-channel-phone-call")).toHaveTextContent("Configure before checking");
    expect(within(screen.getByTestId("row-concierge-channel-phone-call")).getByLabelText("Phone calls live endpoint")).toBeDisabled();
    expect(screen.getByTestId("row-concierge-channel-email")).toHaveTextContent("Ready");
    expect(screen.getByTestId("row-concierge-channel-email")).toHaveTextContent("On");
    expect(screen.getByTestId("row-concierge-channel-email")).toHaveTextContent("Turn off");
    ...
    expect(screen.getByTestId("row-concierge-channel-whatsapp")).toHaveTextContent("Configure");
  ...
  it("keeps live channel controls unavailable until setup is configured and verified", () => {
    renderPage();

    const phoneRow = screen.getByTestId("row-concierge-channel-phone-call");
    expect(within(phoneRow).queryByRole("button", { name: /run check/i })).not.toBeInTheDocument();
    expect(within(phoneRow).queryByRole("button", { name: /^turn on$/i })).not.toBeInTheDocument();
    expect(within(phoneRow).getByText("Turn on unlocks after setup and check.")).toBeInTheDocument();

    const formRow = screen.getByTestId("row-concierge-channel-form-application");
    expect(within(formRow).getByRole("button", { name: /run check/i })).not.toBeDisabled();
    expect(within(formRow).queryByRole("button", { name: /^turn on$/i })).not.toBeInTheDocument();
    expect(within(formRow).getByText("Turn on unlocks after setup and check.")).toBeInTheDocument();

    const emailRow = screen.getByTestId("row-concierge-channel-email");
    expect(within(emailRow).getByText("Ready")).toBeInTheDocument();
    expect(within(emailRow).getByRole("button", { name: /^turn off$/i })).not.toBeDisabled();

    expect(screen.getByTestId("row-concierge-channel-document-upload")).toHaveTextContent("Ready, off");
    expect(within(screen.getByTestId("row-concierge-channel-document-upload")).getByRole("button", { name: /^turn on$/i })).not.toBeDisabled();
  });
  ...
  it("keeps live-ready changes behind a native confirmation form", () => {
    ...
    expect(within(liveForm).getByRole("button", { name: /^turn on$/i })).not.toBeDisabled();
    expect(screen.getByTestId("row-concierge-channel-email")).toHaveTextContent("Ready, off");
  ...
  it("saves adapter setup references and keeps live disabled until the next probe passes", async () => {
    ...
    expect(row).toHaveTextContent("Turn on unlocks after setup and check.");
    expect(within(row).queryByRole("button", { name: /^turn on$/i })).not.toBeInTheDocument();
  });
  ...
  it("posts channel verification through a native form and surfaces failed blockers", () => {
    ...
    expect(screen.getByTestId("row-concierge-channel-form-application")).toHaveTextContent("Turn on unlocks after setup and check.");
    expect(within(screen.getByTestId("row-concierge-channel-form-application")).queryByRole("button", { name: /^turn on$/i })).not.toBeInTheDocument();
  });
