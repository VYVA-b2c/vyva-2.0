...
import { PrescriptionFollowUpVoiceCanvas, RefillVoiceCanvas, isPrescriptionFollowUpEnabled, isRefillCanvasEnabled, parsePrescriptionFollowUpRolloutConfig, parseRefillCanvasRolloutConfig, trackPrescriptionFollowUpEvent, trackRefillCanvasEvent, type PrescriptionFollowUpCopy, type PrescriptionFollowUpSource, type RefillCanvasCopy, type RefillCanvasDraft } from "@/components/voice-canvas";
...
const AdherenceReportScreen = () => {
  ...
  const confirmPrescriptionFollowUp=useCallback(async(source:Readonly<PrescriptionFollowUpSource>,draft:{action:string;missingInformation:string},{signal}:{requestId:number;signal:AbortSignal})=>{if(draft.action==="status"){const response=await apiFetch("/api/concierge/actions/pending",{signal});if(!response.ok)throw new Error(isSpanish?"No pudimos comprobar el estado.":"We could not check the status.");const body=await response.json()as{items?:Array<{id:string}>};return{outcome:"pending" as const,reference:body.items?.find(item=>item.id===source.preparationReference)?.id||source.preparationReference}}const response=await apiFetch("/api/concierge/actions/trigger",{method:"POST",signal,body:JSON.stringify({use_case:"order_medicine",provider_name:source.draft.providerName,found_externally:false,action_summary:"Prescription follow-up preparation ready for review.",action_payload:{preparation_only:true,prescription_follow_up:true,source_preparation_id:source.preparationReference,follow_up_kind:draft.action,missing_information:draft.action==="update"?draft.missingInformation:undefined,confirmation_required_before_contact:true,external_contact_started:false,refill_requested:false,refill_approved:false,ready_for_collection:false,no_dosing_advice:true},language,trigger_source:"manual",auto_start:false})});if(!response.ok){const body=await response.json().catch(()=>null)as{error?:string}|null;throw new Error(body?.error??(isSpanish?"No pudimos preparar el siguiente paso.":"We could not prepare the next step."))}const result=await response.json()as{pendingId?:string};return{outcome:"completed" as const,reference:result.pendingId}},[isSpanish,language]);
