...
const AdherenceReportScreen = () => {
  ...
  const refillCopy=useMemo<RefillCanvasCopy>(()=>({
    ...
    details:{savedProfile:isSpanish?"Guardado en el perfil":"Saved profile",strength:isSpanish?"Concentración":"Strength",providerType:isSpanish?"Tipo de proveedor":"Provider type",quantity:isSpanish?"Cantidad o suministro":"Quantity or supply",routineBoundary:isSpanish?"Solo reposición rutinaria":"Routine refill only",urgentBoundary:isSpanish?"Si es urgente, este flujo se detiene":"Urgent issue stops this flow",noDosingChanges:isSpanish?"Sin cambios de dosis":"No dosing changes",reviewBeforeAction:isSpanish?"Revisar antes de actuar":"Review before action",manualEntry:isSpanish?"Entrada manual":"Manual entry",recommended:isSpanish?"Recomendado":"Recommended"},
  ...
  const refillMedications=useMemo(()=>medications.map((med,index)=>({id:`saved-${index}`,label:med.name,strength:med.dosage,description:med.dosage||undefined,savedLabel:isSpanish?"Medicamento guardado":"Saved medication",profileLabel:isSpanish?"Perfil de medicación":"Medication profile",reviewReminder:isSpanish?"Revisar antes de actuar":"Review before action",recommended:index===0})),[isSpanish,medications]);
  const refillProviders=useMemo(()=>{const saved=(profile?.savedProviders??[]).filter(provider=>provider.name?.trim()&&(/pharmacy|farmacia|doctor|clinic|gp|medic/i.test(`${provider.role??""} ${provider.category??""}`))).map((provider,index)=>{const isPharmacy=/pharmacy|farmacia/i.test(`${provider.role??""} ${provider.category??""}`);return{id:`saved-provider-${index}`,label:provider.name!.trim(),kind:isPharmacy?"pharmacy" as const:"prescriber" as const,description:provider.isTrusted?(isSpanish?"Guardado en tu perfil":"Saved in your profile"):undefined,typeLabel:isPharmacy?(isSpanish?"Farmacia":"Pharmacy"):(isSpanish?"Prescriptor":"Prescriber"),reviewReminder:isSpanish?"Revisar antes de actuar":"Review before action",recommended:index===0}});if(gpName&&!saved.some(item=>item.label===gpName))saved.unshift({id:"saved-gp",label:gpName,kind:"prescriber",description:isSpanish?"Profesional guardado":"Saved clinician",typeLabel:isSpanish?"Prescriptor":"Prescriber",reviewReminder:isSpanish?"Revisar antes de actuar":"Review before action",recommended:true});return saved},[gpName,isSpanish,profile?.savedProviders]);
  const refillContacts=useMemo(()=>[{id:"in-app",label:isSpanish?"Revisar en VYVA":"Review in VYVA",description:isSpanish?"Sin contacto externo":"No external contact",reviewReminder:isSpanish?"Revisar antes de actuar":"Review before action"},{id:"phone",label:isSpanish?"Revisar por teléfono más tarde":"Review by phone later",reviewReminder:isSpanish?"Revisar antes de actuar":"Review before action"},{id:"email",label:isSpanish?"Revisar por correo más tarde":"Review by email later",reviewReminder:isSpanish?"Revisar antes de actuar":"Review before action"}],[isSpanish]);
