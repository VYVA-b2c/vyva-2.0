...
const TOPBAR_LAYOUT_CLASS =
  "grid grid-cols-[40px_1fr_40px] items-center gap-3 px-1 sm:grid-cols-[44px_1fr_44px] sm:px-3";

const ROUND_CONTROL_BASE =
  "vyva-tap relative flex h-9 !min-h-9 w-9 items-center justify-center rounded-full shadow-[0_10px_22px_rgba(36,28,48,0.07)] sm:h-10 sm:!min-h-10 sm:w-10";

const ACTION_CONTROL_BASE =
  "vyva-tap flex h-9 !min-h-9 w-9 items-center justify-center rounded-full text-white shadow-[0_10px_22px_rgba(107,33,168,0.16)] sm:h-10 sm:!min-h-10 sm:w-10";
...
export function HomeMasterProfileControl({
  ...
  return (
    ...
    >
      <VyvaMark className="h-[18px] w-[18px] sm:h-[19px] sm:w-[19px]" variant={isDark ? "white" : "purple"} />
    </button>
  );
}
