...
export function RideVoiceCanvas({copy,places,dateChoices,voiceCommands,onConfirmRide,onDone,onCancel,storageKey="vyva.rideCanvas.v1",initialState}:RideVoiceCanvasProps){
  const restore=()=>{
    if(initialState) return initialState;
    try{const value=sessionStorage.getItem(storageKey);if(value){const parsed=JSON.parse(value);if(isRestorableRideState(parsed)) return parsed;}}catch{/* storage may be unavailable */}
  ...
  useEffect(()=>{
    ...
    onConfirmRide(Object.freeze({...state.draft}),{requestId:request.id,signal:controller.signal})
      .then(result=>{if(!controller.signal.aborted) dispatch({type:"RESOLVE",requestId:request.id,reference:result.reference});})
      .catch(error=>{if(!controller.signal.aborted) dispatch({type:"REJECT",requestId:request.id,message:error instanceof Error?error.message:undefined});});
  },[state.step,state.requestId,state.draft,onConfirmRide]);
  ...
  useEffect(()=>{
    const handler=(event:Event)=>{
      ...
      if(voiceCommands.cancel.some(command=>text===normalized(command))){dispatch({type:"CANCEL"});onCancel?.();}
      ...
      else if(voiceCommands.confirm.some(command=>text===normalized(command))&&state.step==="review") dispatch({type:"CONFIRM"});
      else if(voiceCommands.retry.some(command=>text===normalized(command))&&state.step==="blocked") dispatch({type:"RETRY"});
  ...
  },[state.step,places,dateChoices,voiceCommands,onCancel,viewModel.title]);

  return <div ref={rootRef} data-testid="ride-voice-canvas" data-step={state.step}><VoiceCanvasScene viewModel={viewModel} onChoice={choose} onPrimary={primary} onSecondary={secondary} onTextChange={value=>dispatch({type:state.step==="dateTime"?"CHANGE_TIME":"CHANGE_ADDRESS",value})}/><span className="sr-only" aria-live="polite" aria-atomic="true">{viewModel.statusLabel||viewModel.title}</span></div>;
}
