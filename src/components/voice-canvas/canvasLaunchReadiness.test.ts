...
describe("Canvas launch readiness manifest", () => {
  ...
  it("keeps the real-device sign-off matrix aligned with launch scope", () => {
    ...
    for (const requiredCheck of [
      ...
      "Final preflight should use `--date=YYYY-MM-DD`",
      "date shortcut loads the launch run plan",
      "copy clarity, recovery behavior, real-use, or entry-surface evidence is missing or invalid",
      "rollback owner handoff is missing or invalid",
      "must share one QA run date",
  ...
  it("provides a sanitized evidence packet for staging QA execution", () => {
    ...
    for (const artifactSet of [
      ...
      "Rollback owner handoff artifacts",
      "owner/distinct-backup, decision window, rollback trigger, sanitized endpoint/fallback/open-session evidence, and privacy boundary",
      "Operations/rollback owner and distinct backup owner, decision window, rollback trigger",
      "npm run --silent canvas:qa:rollback-owner -- --template --output=artifacts/voice-canvas/YYYY-MM-DD-rollback-owner-handoff.md",
      "npm run --silent canvas:qa:rollback-owner -- --input=artifacts/voice-canvas/YYYY-MM-DD-rollback-owner-handoff.md --json --output=artifacts/voice-canvas/YYYY-MM-DD-rollback-owner-validation.json",
      "same `YYYY-MM-DD` QA run date",
  ...
  it("provides a per-flow real-device run sheet for staging QA execution", () => {
    ...
    for (const requiredRunSheetCoverage of [
      ...
      "records only sanitized endpoint status, cache-control, timing, expected state, `enabled`, `rolloutPercent`, recognized payload keys, unexpected-key count",
      "Feature endpoint manual trace evidence",
      "npm run --silent canvas:qa:features -- --trace-template",
