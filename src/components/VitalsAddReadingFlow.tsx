...
export default function VitalsAddReadingFlow({
  ...
  const back = stage === "vital" ? onBack : stage === "tracked" || stage === "method" ? resetToVital : () => setStage("method");
  const selectedMeta = selectedSignal ? VITALS_SIGNAL_CATALOG[selectedSignal] : null;

  return (
    <section className="rounded-[28px] border border-[#E7DDF0] bg-[#FFFCF8] p-5 shadow-[0_18px_46px_rgba(53,28,87,0.08)]" data-testid="vitals-add-flow">
      <header className="mb-6 flex items-center gap-4">
        <button type="button" onClick={back} className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full border border-[#E7DDF0] bg-white text-[#6B21A8]" aria-label="Back">
          <ArrowLeft className="h-5 w-5" />
        </button>
        <div>
          <p className="font-body text-[12px] font-black uppercase tracking-[0.14em] text-[#7C3AED]">Add a reading</p>
          <h2 className="font-display text-[28px] font-bold leading-tight text-[#27152F]">
        ...
        loadingContext ? (
          ...
          <div className="space-y-6">
            {groupedSignals.map(({ group, signals }) => (
              <div key={group}>
                <p className="mb-2 font-body text-[12px] font-black uppercase tracking-[0.13em] text-[#7A6A80]">{GROUP_LABELS[group]}</p>
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  {signals.map((signal) => {
                    const meta = VITALS_SIGNAL_CATALOG[signal];
                    return (
                      <button key={signal} type="button" onClick={() => chooseSignal(signal)} className="flex min-h-[68px] items-center gap-3 rounded-[20px] border border-[#E7DDF0] bg-white px-4 text-left shadow-[0_5px_16px_rgba(53,28,87,0.04)]" data-testid={`button-vital-${signal}`}>
                        <span className="flex h-10 w-10 items-center justify-center rounded-[14px] bg-[#F3E8FF] text-[#7C3AED]"><HeartPulse className="h-5 w-5" /></span>
                        <span className="min-w-0 flex-1">
                          <span className="block font-body text-[16px] font-black text-[#27152F]">{meta.label}</span>
                          <span className="block font-body text-[12px] font-bold text-[#7A6A80]">{meta.unit || "Yes or no"}</span>
                        </span>
                        <ChevronRight className="h-5 w-5 text-[#A78BBA]" />
                      </button>
                    );
                  })}
                </div>
      ...
      {stage === "method" ? (
        <div>
          <p className="mb-4 font-body text-[16px] font-bold text-[#6B5B72]">Choose the easiest way to add it. You will confirm before anything is saved.</p>
          <div className="grid gap-3 sm:grid-cols-2" data-testid="vitals-method-picker">
            {methods.map((method) => {
              const detail = METHOD_DETAILS[method];
              const Icon = detail.Icon;
              return (
                <button key={method} type="button" onClick={() => chooseMethod(method)} className="flex min-h-[88px] items-center gap-4 rounded-[22px] border border-[#E0D1EC] bg-white p-4 text-left shadow-[0_7px_20px_rgba(53,28,87,0.05)]" data-testid={`button-method-${method}`}>
                  <span className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-[16px] bg-[#F3E8FF] text-[#7C3AED]"><Icon className="h-6 w-6" /></span>
                  <span className="min-w-0">
                    <span className="block font-body text-[16px] font-black text-[#27152F]">{detail.label}</span>
                    <span className="mt-1 block font-body text-[12px] font-bold leading-snug text-[#7A6A80]">{detail.hint}</span>
                  </span>
                </button>
              );
            })}
      ...
      {stage === "capture" && selectedMethod === "phone_camera" && selectedSignal ? (
        <div className="overflow-hidden rounded-[24px] border border-[#E0D1EC] bg-white p-3">
      ...
      {stage === "capture" && selectedMethod === "device_photo" ? (
        <label className="flex min-h-[190px] cursor-pointer flex-col items-center justify-center gap-3 rounded-[24px] border border-dashed border-[#B997D4] bg-[#F8F1FC] p-6 text-center">
          {busy ? <Loader2 className="h-8 w-8 animate-spin text-[#7C3AED]" /> : <FileImage className="h-9 w-9 text-[#7C3AED]" />}
          <span className="font-display text-[22px] font-bold text-[#27152F]">Take or upload a device photo</span>
          <span className="font-body text-[13px] font-bold text-[#7A6A80]">Keep the full number and unit clearly visible.</span>
      ...
      {stage === "capture" && (selectedMethod === "manual" || selectedMethod === "voice") ? (
        <div className="rounded-[24px] border border-[#E0D1EC] bg-white p-4">
          ...
          <textarea value={inputText} onChange={(event) => setInputText(event.target.value)} placeholder={selectedMeta ? `${selectedMeta.label} ${selectedMeta.unit}` : "Type the reading"} className="min-h-[120px] w-full rounded-[18px] border border-[#E0D1EC] bg-[#FFFCF8] px-4 py-3 font-body text-[18px] font-bold text-[#27152F] outline-none focus:border-[#7C3AED]" />
