                ...
                <div className="mt-2 space-y-2">
                  {item.changes.map((change) => (
                    <div key={change.criterion} className="grid gap-1 border-t border-[#EEE8E0] pt-2 sm:grid-cols-[120px_minmax(0,1fr)] sm:gap-3">
                      ...
                      <div className="flex min-w-0 items-center gap-2 font-body text-[12px] font-bold text-vyva-text-1">
                        <span className="min-w-0 break-words"><span className="text-vyva-text-3">{copy.before}: </span>{change.before.value || copy.unknown}</span>
                        <ArrowRight size={13} className="shrink-0 text-vyva-purple" aria-hidden="true" />
                        <span className="min-w-0 break-words"><span className="text-[#0F766E]">{copy.now}: </span>{change.after.value || copy.unknown}</span>
                      </div>
                    </div>
                  ))}
                </div>
