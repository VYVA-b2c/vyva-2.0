...
test("synchronizes voice and restores an interrupted draft", async ({ page }) => {
  await page.evaluate(() => window.dispatchEvent(new CustomEvent("vyva:voice-user-message", { detail: { text: "arrange a ride", transcriptEntry: { from: "user", text: "arrange a ride" } } })));
  ...
  await page.reload();
  await expect(page.getByLabel("Destination address")).toHaveValue("99 Garden Road");
});

test("supports keyboard-only use", async ({ page }) => {
  await page.getByRole("button", { name: "Arrange a ride" }).focus();
  await page.keyboard.press("Enter");
  await expect(page.getByRole("heading", { name: "Where would you like to go?" })).toBeFocused();
  await page.getByRole("button", { name: "Home" }).focus();
  await page.keyboard.press("Enter");
  await expect(page.getByRole("heading", { name: "When should the ride arrive?" })).toBeFocused();
});

test("keeps long Spanish labels inside a mobile viewport", async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto("/voice-canvas-integration.html?locale=es");
  await page.getByRole("button", { name: "Preparar un viaje" }).click();
  await expect(page.getByRole("button", { name: "Centro médico y de bienestar Riverside" })).toBeVisible();
