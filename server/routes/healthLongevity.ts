...
import { buildPreventionFocus, type PreventionLoopContext, type PreventionVitalReading } from "../lib/longevityFocus.js";
import {
  medicationAdherence,
...
// Longevity is the canonical product name. Keep /prevention as a compatibility
// alias while deployed clients and workflow receipts migrate.
router.get(["/longevity", "/prevention"], async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("[health-longevity] GET longevity focus failed", err);
    return res.status(500).json({ error: "Failed to build longevity focus" });
  }
});
