...
export const CONCIERGE_SPECIALIST_FLAG_ID = "flag.concierge.specialist" as const;
export const CONCIERGE_SPECIALIST_FLAG_VERSION = "1.0.0" as const;

export type ConciergeSpecialistEffectiveMode =
  | "legacy_only"
  | "specialist_preview";

export type ConciergeSpecialistFlagResolution = {
  flagId: typeof CONCIERGE_SPECIALIST_FLAG_ID;
  flagVersion: typeof CONCIERGE_SPECIALIST_FLAG_VERSION;
  effectiveMode: ConciergeSpecialistEffectiveMode;
  selected: boolean;
  reasonCode:
    | "default_legacy"
    | "explicit_legacy"
...
export type ConciergeSpecialistFlagInput = {
  env?: Readonly<Record<string, string | undefined>>;
  userRef: string;
  cohortKey?: string;
};

const MODE_ENV = "VYVA_CONCIERGE_SPECIALIST_MODE";
const ROLLOUT_ENV = "VYVA_CONCIERGE_SPECIALIST_ROLLOUT_BPS";
const ALLOW_USERS_ENV = "VYVA_CONCIERGE_SPECIALIST_ALLOW_USERS";
const DENY_USERS_ENV = "VYVA_CONCIERGE_SPECIALIST_DENY_USERS";
const ALLOW_PRODUCTION_ENV = "VYVA_CONCIERGE_SPECIALIST_ALLOW_PRODUCTION";

function invalid(): ConciergeSpecialistFlagResolution {
  return {
    flagId: CONCIERGE_SPECIALIST_FLAG_ID,
    flagVersion: CONCIERGE_SPECIALIST_FLAG_VERSION,
...
function legacy(reasonCode: ConciergeSpecialistFlagResolution["reasonCode"]): ConciergeSpecialistFlagResolution {
  return {
    flagId: CONCIERGE_SPECIALIST_FLAG_ID,
    flagVersion: CONCIERGE_SPECIALIST_FLAG_VERSION,
    effectiveMode: "legacy_only",
...
function selected(reasonCode: ConciergeSpecialistFlagResolution["reasonCode"]): ConciergeSpecialistFlagResolution {
  return {
    flagId: CONCIERGE_SPECIALIST_FLAG_ID,
    flagVersion: CONCIERGE_SPECIALIST_FLAG_VERSION,
...
function bucket(flagFacts: { userRef: string; cohortKey: string }): number {
  const digest = createHash("sha256")
    .update(`${CONCIERGE_SPECIALIST_FLAG_ID}:${CONCIERGE_SPECIALIST_FLAG_VERSION}:${flagFacts.userRef}:${flagFacts.cohortKey}`)
    .digest("hex")
    .slice(0, 8);
  return Number.parseInt(digest, 16) % 10_000;
}

export function resolveConciergeSpecialistFlag(
  input: ConciergeSpecialistFlagInput,
): ConciergeSpecialistFlagResolution {
