...
const MAIN_VYVA_RETURNING_APP_OPEN: VoiceConversationPlan = {
  ...
  opening_instruction:
    "This is a returning VYVA voice session. Greet briefly, use the user's name if available, use relationship continuity, memory, app insight, preferences, recent activity, social context, and upcoming events to suggest one useful next action, then invite the user to choose.",
  steps: [
    ...
    "Suggest one useful next action based on the current app entry point, conversation plan, app insight, and available context.",
