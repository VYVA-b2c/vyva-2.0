...
function maxStatus(a: SafetyStatus, b: SafetyStatus): SafetyStatus {
...
export function buildDailySafetyCheck(input: DailySafetyInput): DailySafetyCheck {
  ...
  for (const summary of input.signalSummary) {
    ...
    if (!finding) continue;
    status = maxStatus(status, finding.status);
  ...
  if (triage) {
    status = maxStatus(status, triage.status);
  ...
  if (medication?.scheduledToday && medication.takenToday < medication.scheduledToday) {
    status = maxStatus(status, "recheck");
  ...
  if ((medication?.missedOrLate30 ?? 0) >= 3) {
    status = maxStatus(status, "share_with_caregiver");
  ...
  const riskTier = statusToRiskTier(status) === "none" ? riskTierForScore(riskScore) : statusToRiskTier(status);
  ...
  return {
    ...
    caregiver_note: STATUS_RANK[status] >= STATUS_RANK.share_with_caregiver
...
export function mergeAiSafetySuggestion(base: DailySafetyCheck, ai: AiSafetySuggestion | null | undefined): DailySafetyCheck {
  if (!ai) return base;
  ...
  const mergedStatus = aiStatus ? maxStatus(base.safety_status, aiStatus) : base.safety_status;
  const mergedScore = Math.max(base.risk_score, aiScore, statusToRiskScore(mergedStatus));
  const riskTier = statusToRiskTier(mergedStatus) === "none" ? riskTierForScore(mergedScore) : statusToRiskTier(mergedStatus);
