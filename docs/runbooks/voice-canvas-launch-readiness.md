...
For evidence artifact inventory rows, map the sanitized artifact bucket to the exact evidence it proves, including environment/flag, real-device, interaction, behavior recovery, feature endpoint, task hub, copy/accessibility, analytics signal, and analytics privacy artifacts. Each reference must point to concrete sanitized screenshots, photos, recordings, logs, traces, captures, dashboards, queries, or links with no personal details, and each row must include a QA/reviewer note with a non-future `YYYY-MM-DD` date.
