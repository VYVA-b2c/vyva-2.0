...
6. Record the fallback and no-side-effect result in `docs/audits/voice-canvas-real-device-run-sheet.md`, `docs/audits/voice-canvas-real-device-evidence-packet.md`, and `docs/audits/voice-canvas-real-device-qa-matrix.md`; final launch sign-off still requires `npm run canvas:qa:preflight -- --final --date=YYYY-MM-DD` with the same-date launch evidence bundle: run plan, enabled and rollback-disabled endpoint artifacts, analytics, copy clarity, recovery behavior, real-use, entry-surface, and rollback-owner handoff artifacts.
