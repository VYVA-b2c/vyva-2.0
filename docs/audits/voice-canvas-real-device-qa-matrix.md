...
Each flow must be completed or safely exited using voice, touch, and keyboard. For final sign-off, each cell must name the interaction mode tested and state that the flow was completed or safely exited rather than only saying that QA passed. Evidence must include a dated QA or reviewer note.
...
For final sign-off, each result cell must name the specific behavior checked, not just say that QA passed. Evidence cells must point to matching screenshots, keyboard/focus notes, screen-reader announcement notes, reduced-motion notes, or copy read-through evidence.
