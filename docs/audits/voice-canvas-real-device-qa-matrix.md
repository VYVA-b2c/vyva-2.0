...
Keep `Status` as **pending execution** until this matrix is fully filled. For final launch sign-off, change it to **ready for launch** only after every `Pending` cell is replaced with a result/evidence note and all final sign-off roles have a name, date, and decision. Then run:
