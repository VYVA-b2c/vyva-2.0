...
Use the QA run date in the output path. The preflight delegates to the run-sheet, matrix, and evidence-packet validators, preserves existing artifacts unless `--force` is explicit, and does not call feature endpoints, analytics, bookings, calls, messages, navigation, or data writes. After all standard same-date evidence is filled, run `npm run canvas:qa:preflight -- --final --date=YYYY-MM-DD` as the combined final local gate. The date shortcut loads the launch run plan, enabled and rollback-disabled endpoint artifacts, analytics, copy clarity, recovery behavior, real-use, entry-surface, and rollback-owner handoff artifacts from `artifacts/voice-canvas/`. Those artifacts must all come from one deployed QA origin, and endpoint auth metadata must match the launch run plan without storing credential names, values, tokens, cookies, passwords, or API keys.
...
- `npm run --silent canvas:qa:preflight -- --json --output=artifacts/voice-canvas/YYYY-MM-DD-launch-preflight.json` has been captured before QA starts, and `npm run canvas:qa:preflight -- --final --date=YYYY-MM-DD` passes after the run sheet, packet, matrix, launch run plan, sanitized feature endpoint evidence, sanitized analytics evidence, sanitized copy clarity evidence, sanitized recovery behavior evidence, sanitized real-use evidence, sanitized entry-surface evidence, and rollback owner handoff are complete.
