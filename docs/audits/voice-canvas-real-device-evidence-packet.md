...
| Launch run plan artifacts | `voice-canvas/preflight/<YYYY-MM-DD>/launch-evidence-run.json` | Same date launch artifact run plan for endpoint, analytics, copy-clarity, recovery-behavior, real-use, entry-surface, rollback-owner, run-sheet, matrix, packet, and final preflight evidence | Pending |
