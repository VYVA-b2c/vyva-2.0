...
| Mobile, tablet, and desktop presentation plus voice/touch/keyboard operation | Playwright screenshots for ride, appointment, refill, shopping, provider reply, and task hub resume under `src/dev/voice-canvas/`; task hub component coverage in `src/pages/ConciergeTaskInboxPage.test.tsx`; explicit interaction-mode sign-off gate | Browser evidence complete; physical-device and interaction-mode pass pending |
| Refresh/reconnect, app exit/reopen, and voice interruption restore editable work without resubmitting in-flight work | Flow component tests, task hub tests, `canvasPlatformCompliance.test.ts`, launch runbook restore rules, and explicit app-exit/reopen and voice-interruption sign-off gates | Automated evidence complete; deployed reconnect/app-exit/interruption pass pending |
| Back, cancel, and clear exit paths are safe | Flow component tests, task hub detail-exit tests, and explicit browser-back preservation/no-write sign-off gate | Automated evidence complete; deployed browser-back pass pending |
| Recoverable failures provide retry or exit without submitting extra work | Flow blocked-state tests, launch runbook failure scenario, and explicit recoverable-failure retry sign-off gate | Automated evidence complete; deployed recoverable-failure pass pending |
...
2. On a real phone, tablet, and desktop, run ride, appointment, refill, shopping, provider reply, and task hub resume using voice, touch, and keyboard where supported. Record results in `docs/audits/voice-canvas-real-device-qa-matrix.md`.
