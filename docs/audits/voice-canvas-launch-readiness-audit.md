...
The matrix is protected by `src/components/voice-canvas/canvasLaunchSignoff.test.ts`: it may stay **pending execution** while real-device QA is incomplete, but a **ready for launch** status is rejected unless every required environment, device, behavior, rollback, copy/accessibility, privacy, and sign-off row remains present; all required evidence is filled with passing results; no row contains a failed/blocked/not-ready result; and every final sign-off role has a name, a `YYYY-MM-DD` date, and an approved-for-launch decision.
