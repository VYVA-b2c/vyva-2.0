...
- **Implemented slice:** a default-disabled Health caregiver/operator escalation projection layer for `health.preventive_check@1.0.0`. The Health completion path may propose escalation only after a newly persisted authoritative completion whose result has `flag_caregiver: true`; the Stage 9 adapter then resolves the pilot flag, re-reads current Stage 9 purpose consent, uses the existing Health caregiver domain access helper for caregiver candidates, evaluates operator/admin visibility as a separate audience, and writes only minimized projection rows for allowed decisions.
- **Files affected:** caregiver/operator escalation policy, projection adapter/store, migration 0082, Drizzle schema metadata, Health completion hook dependency, check-in route adapter wiring, focused authorization/projection/Health integration/PostgreSQL tests, CI PostgreSQL 16 freeze-proof job, and `docs/HEALTH_CAREGIVER_OPERATOR_ESCALATION.md`.
- **New files:** escalation policy, caregiver/operator projection store, migration 0082, projection documentation and Task 14 tests.
- **Dependencies:** consent policy and Health flow.
- **Flag:** `flag.health.caregiver_operator_escalation`, version `1.0.0`, configured through `VYVA_HEALTH_CAREGIVER_OPERATOR_ESCALATION_*`. Absent/empty config defaults to `disabled`; malformed mode, allowlist, denylist, rollout, production-gate, environment, user ID or cohort values fail closed. `pilot` is required before projections can be created.
- **Consent:** uses only the explicit `profiles.data_sharing_consent.health_caregiver_operator_escalation` purpose object. Push consent, outbound-call consent, semantic-memory consent, generic engagement preferences, caregiver relationship and existing caregiver-alert booleans do not authorize Stage 9. Revocation blocks future projection creation, projection disclosure and acknowledgement.
- **Projection:** `health_caregiver_operator_escalation_projections` stores subject/profile IDs, target audience, caregiver target actor where applicable, Health flow ID/version/instance, source Health event ID, optional existing alert reference, completion reference, answer digest, authorization/consent reason codes, minimized safe summary, deterministic idempotency key, semantic digest and acknowledgement fields. Rows are projections only and are not authoritative over Health state, alert meaning, queue status, caregiver permissions or consent.
- **Disclosure boundary:** raw projection persistence is internal. Reads and acknowledgement use an authorized Stage 9 boundary that accepts authenticated actor identity only, reloads current Stage 9 consent, reloads current caregiver Health-domain access or server-owned operator/admin role authority, and returns generic no-data/not-found outcomes on denial. Caller-supplied role strings cannot grant operator access, and projection creation-time consent is not permanent disclosure authority.
- **Acceptance:** unauthorized escalation creates no readable projection; caregiver ID guessing and cross-user acknowledgement fail closed; operator projection visibility is separate from caregiver-specific rights and linked to the same Health flow/audit evidence; consent revocation, caregiver-access loss and operator-role loss after projection creation block later disclosure and acknowledgement while preserving the auditable projection row.
- **Tests:** role/consent matrices, explicit unrelated-consent denial, descriptor-safe consent parsing, projection idempotency, cross-user/IDOR visibility rejection, operator-vs-caregiver access separation, acknowledgement idempotency, revoked-consent read/acknowledgement rejection, current caregiver-access denial, current operator-role denial, Health hook ordering, migration/schema parity and PostgreSQL store proofs.
- **Observability:** projection rows retain stable decision reason codes, actor/audience metadata, flow/audit IDs and acknowledgement metadata. Safe summaries contain only stable classification/reason codes and explicitly retain no raw Health answer content.
- **Rollback:** disable the Stage 9 flag. Current alert rows, current queue views, current caregiver permissions and Health completion behavior remain; projection data remains inert/auditable.
- **Risk:** critical.
- **Do not change:** existing caregiver permissions, current alert rows, or queue status meanings.
- **Real proof:** PostgreSQL freeze proof follows the Task 10/11/13 disposable-database CI pattern with PostgreSQL 16 database `vyva_task14_test`, runs migration 0082, resets schema, applies the authoritative baseline via `npm run db:push:manual`, then runs the real projection-store proof covering concurrent duplicate projection creation, conflict classification, acknowledgement idempotency, cross-user rejection, current-consent revocation blocking durable disclosure/acknowledgement, current caregiver-access denial, current operator-role denial, and existing alert/queue row immutability. Local PostgreSQL tests require `TASK14_POSTGRES_URL` to point to a scratch database whose name contains `task14` and `test`, `tmp`, `ci` or `scratch`.
- **Deferred:** no public caregiver/operator read API, no UI/dashboard rendering, no existing alert mutation, no concierge queue status integration, no emergency-path policy change and no broader runtime rollout approval.
