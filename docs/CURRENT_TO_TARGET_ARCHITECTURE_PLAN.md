...
- **Reuse:** prevention/check-in routes, screens, voice context, caregiver logic.
- **Files likely affected:** new Health specialist/flow files; thin adapters around `healthPrevention.ts` and `checkins.ts`; flagged projection in the check-in screen.
- **New files:** Health Specialist adapter and Flow definition.
- **Dependencies:** Stages 1–3.
- **Flag:** user/percentage allowlist; default off.
- **Acceptance:** voice/touch/text parity, safety precedence, structured save, completion.
- **Tests:** full flow and existing check-in regressions.
- **Observability:** flow transition and specialist-validation events.
- **Rollback:** route flagged user to existing `CheckHowIFeelScreen` behavior.
- **Risk:** high.
